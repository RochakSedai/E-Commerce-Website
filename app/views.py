...
def mobile(request, data=None):
    ...
    elif data == 'MI' or data == 'Oppo' or data == 'SamSung':
        mobiles = Product.objects.filter(category='M').filter(brand=data)
    elif data == 'below':
        mobiles = Product.objects.filter(category='M').filter(discounted_price__lt=23000)
    elif data == 'above':
        mobiles = Product.objects.filter(category='M').filter(discounted_price__gt=23000)   
...
class ProfileView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            reg = Customer(user=usr, name=name, locality=locality, city=city, zone=zone, zipcode=zipcode)
            reg.save()
